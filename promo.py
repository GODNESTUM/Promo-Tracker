...
#     st.markdown(hide_streamlit_style, unsafe_allow_html=True)
    ...
    st.sidebar.selectbox("Added_by", options = list(data_df.Added_by.unique()),key= 'sb_user')
    ...
    sb_partner = st.sidebar2.selectbox("Partner", options = list(data_df.Partner_Name[data_df.Added_by.isin([sb_user])].unique()),key = 'sb_partner')#, on_change= promo_1.clear_session(st.session_state))
    ...
    sb_ts = st.sidebar3.selectbox("Franchise", options = list(data_df.Title_Series[(data_df.Partner_Name.isin([sb_partner]))&(data_df.Added_by.isin([sb_user]))].unique()),key = 'sb_ts')
